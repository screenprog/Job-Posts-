package com.screenprog.SpringBootRest.repo;
...
public interface JobRepo extends JpaRepository<JobPost, Integer> {
    ...
    @Query
    List<JobPost> findByPostProfileContainingOrPostDescContaining(String jobPost, String jobDesc);
}
    ...
    // DevOps Engineer Job Post
        *//*jobs.add(
        ...
        jobs.add(new JobPost(15, "Content Writer",
                ...
                List.of("Content Writing", "Copy writing", "SEO", "Creative Writing")));
...
public Optional<JobPost> getJob(int id) {
    return jobs.stream()
            .filter(jobPost -> jobPost.getPostId() == id)
            .findFirst();

}

public void updateJob(JobPost jobPostToUpdate) {
    getJob(jobPostToUpdate.getPostId())
            .ifPresent(job -> jobs.set(jobs.indexOf(job), jobPostToUpdate));
}

public String deleteJob(int postId) {
    Optional<JobPost> jobPostToDelete = getJob(postId);

    if(jobPostToDelete.isPresent()){
        jobPostToDelete.ifPresent(job -> jobs.remove(job));
        return "Deleted successfully";
    }
    return String.format("Post id '%d' do not exist", postId);
}*/
